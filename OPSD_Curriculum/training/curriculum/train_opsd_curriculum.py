...
from curriculum_schedule import build_schedule, derive_stage_maps, save_schedule_meta, OUT_DIR
from curriculum_collator import CurriculumDataCollator
from curriculum_trainer import CurriculumOPSDTrainer
from curriculum_monitor import CurriculumMonitorCallback
...
class CurriculumScriptArguments(CustomScriptArguments):
    ...
    arm: str = field(default="main", metadata={"help": "'main' (diff x cluster) or 'diffonly'."})
    ...
    curriculum_T: int = field(default=480, metadata={"help": "Total optimizer steps (arm-common for full=480)."})
...
def main():
    ...
    T = int(script_args.curriculum_T)
    B_glob = int(script_args.curriculum_B_glob)

    is_main = os.environ.get("LOCAL_RANK", "0") == "0"

    # ---------------- schedule (CPU, deterministic) ----------------
    schedule, meta = build_schedule(
        Path(script_args.stages_json), Path(script_args.row_table),
        script_args.arm, T, B_glob, script_args.curriculum_seed,
    )
    stage_per_pos, stage_per_optstep = derive_stage_maps(meta, B_glob)
    assert len(schedule) == len(stage_per_pos), (len(schedule), len(stage_per_pos))
    assert len(stage_per_optstep) == T, (len(stage_per_optstep), T)
    if is_main:
        save_schedule_meta(meta, f"{script_args.arm}_run")
        print(f"[curriculum] arm={script_args.arm} T={T} B_glob={B_glob} "
              f"schedule_len={len(schedule)} stages={meta['num_stages']}", flush=True)
    ...
    training_args.max_steps = T                   # exactly one pass over T*B_glob
    ...
    if is_main:
        ...
        wandb.init(
            ...
            name=run_name,
            config={
                "arm": script_args.arm, "curriculum_T": T, "B_glob": B_glob,
                "effective_batch_size": eff_bs, "num_processes": num_processes,
    ...
    # curriculum monitor (stage_respected gate; source of truth = schedule)

    trainer.add_callback(CurriculumMonitorCallback(trainer, stage_per_optstep,
                                                   attach_gold=script_args.attach_gold))
