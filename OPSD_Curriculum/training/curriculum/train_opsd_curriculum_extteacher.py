#!/usr/bin/env python3
"""
train_opsd_curriculum_extteacher.py — EXPERIMENTAL (2026-07-12)
========================
train_opsd_curriculum_manifest_once.py 의 복사 변형: 티처를 "학생 자신(LoRA-off)+gold"
대신 **별도 frozen 모델(teacher_model_name_or_path, 예: Qwen3-14B)** 로 교체.
⚠️ 원본 파이프라인/opsd_src 는 수정하지 않음(OPSD_original 동일 유지) — self-distillation
실험은 계속 원본 진입점으로. 이 파일과의 차이는 딱 3곳:
  (a) ExtTeacherScriptArguments.teacher_model_name_or_path 필드 추가
  (b) CurriculumOPSDTrainer → ExtTeacherCurriculumOPSDTrainer
  (c) trainer 에 teacher_model_name_or_path kwarg 전달
나머지는 원본과 라인 단위 동일. 원본이 바뀌면 이 복사본도 갱신할 것.
"""
import os
...
from extteacher_trainer import ExtTeacherCurriculumOPSDTrainer   # (b) 변경점
...
@dataclass
class ExtTeacherScriptArguments(CustomScriptArguments):
    """OPSD script args + curriculum knobs + external teacher. (원본 CurriculumScriptArguments와
    동일 필드 + teacher_model_name_or_path)"""
    ...
    stage_teacher_update: bool = field(default=False, metadata={"help": "(원본 호환용; ext-teacher 에서는 사용 금지 — frozen external teacher 와 상호배타.)"})
    # (a) 변경점: 외부 티처
    teacher_model_name_or_path: str = field(
        default=None,
        metadata={
            "help": "SEPARATE frozen teacher model (e.g. Qwen/Qwen3-14B) for cross-model distillation. "
            "Required by this entrypoint. Must share the student's tokenizer/vocab (Qwen3 family: 151936). "
            "Mutually exclusive with fixed_teacher / use_ema_teacher."
        },
    )


def main():
    parser = TrlParser((ExtTeacherScriptArguments, GOLDConfig, ModelConfig))
    script_args, training_args, model_args = parser.parse_args_and_config()

    assert script_args.stages_json, "stages_json is required"
    assert script_args.teacher_model_name_or_path, "teacher_model_name_or_path is required for the ext-teacher entrypoint"
    assert not script_args.stage_teacher_update, "stage_teacher_update is incompatible with a frozen external teacher"
    ...
    if is_main:
        ...
        print(f"[curriculum-extteacher] arm={script_args.arm} spec={meta.get('spec')} "
              f"T={T} B_glob={B_glob} schedule_len={len(schedule)} "
              f"tail={meta['tail_size']} stages={meta['num_stages']} "
              f"order={script_args.within_stage_order} passes={script_args.curriculum_passes} "
              f"teacher={script_args.teacher_model_name_or_path}", flush=True)
    ...
    if is_main:
        ...
        wandb.init(
            ...
            config={
                ...
                "teacher_model_name": script_args.teacher_model_name_or_path,
    ...
    trainer = ExtTeacherCurriculumOPSDTrainer(   # (b)(c) 변경점
