...
#
# QUICK EVAL cond3_ours checkpoint-810 — Qwen3-8B + LoRA(r=64) on L40S x2 (TP=2).
...
run_eval() {
  local ds="$1" valn="$2"
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL cond3_ours_step810  dataset=$ds  val_n=$valn  temp=1.0  thinking=OFF  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_cond3_ours_step810_nonthink_valn${valn}.json"
