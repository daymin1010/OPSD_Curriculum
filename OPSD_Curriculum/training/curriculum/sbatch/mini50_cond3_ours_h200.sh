...
#SBATCH --gres=gpu:2
#SBATCH --mem=200G
#SBATCH --job-name opsd_mini50_cond3_ours_h200
...
# Uses stages_cond3_ours_C2_mini50.json (N=1603, T_total~51).
# save_steps=10 → ckpts at 10/20/30/40/50 (stage starts + last).
# ============================================================
set -euo pipefail
...
DIFF_JSON=$STAGES_TIERED/stages_cond2_diff_mini50.json
OURS_JSON=$STAGES_TIERED/stages_cond3_ours_C2_mini50.json
RUN_CONFIG=mini50_cond3_ours_h200
...
if [ ! -f "$ROW" ]; then
  echo "[mini50] missing row table: $ROW ; run phase0 first" >&2
  exit 2
fi
...
accelerate launch \
    ...
    --config configs/mini50_8b_h200.yaml \
    --arm cond3_ours_mini50 \
