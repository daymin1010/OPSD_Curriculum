...
#SBATCH --mem=200G
#SBATCH --job-name opsd_cur_eval_diffonly_hmmt25
...
#
# EVAL ②-A (diff-only) — Qwen3-8B + LoRA(r=64) on L40S (48GB) x2 (TP=2).
# Uses OPSD canonical eval harness (opsd_src/eval/evaluate_math.py).
# thinking ON (default), val_n=12 (Avg@12), temp=1.0, datasets: aime24/aime25/hmmt25.
...
CKPT=$REPO/checkpoints/opsd_curriculum/full_8b/full_diffonly_h200   # final adapter (step480, top-level)
OUTDIR=$REPO/outputs/eval_opsd_curriculum/diffonly
...
for DS in hmmt25; do
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL diff-only  dataset=$DS  val_n=12  temp=1.0  thinking=ON  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --output_file "$OUTDIR/${DS}_diffonly_thinking_valn12.json"
