...
#SBATCH --gres=gpu:2
#SBATCH --mem=200G
#SBATCH --job-name opsd_quarter_cond2_diff_q4_h200
...
# ============================================================
# QUARTER diff — manifest_once fair 1-pass OPSD curriculum (1/4 scale)
# Qwen3-8B, H200 x2, B_glob=32 via pd2*ga8*ws2.
# Uses stages_cond2_diff_q4.json, ~7.2k problems, T~225.
# save_steps=10, save_total_limit=30 to preserve checkpoints.
# ============================================================
set -euo pipefail
...
DIFF_JSON=$STAGES_TIERED/stages_cond2_diff_q4.json
OURS_JSON=$STAGES_TIERED/stages_cond2_diff_q4.json
RUN_CONFIG=quarter_cond2_diff_q4_h200
...
if [ ! -f "$ROW" ]; then
  echo "[quarter] missing row table: $ROW ; run phase0 first" >&2
  exit 2
fi
...
accelerate launch \
    ...
    --main_process_port 12974 \
    train_opsd_curriculum_manifest_once.py \
    --config configs/quarter_8b_h200.yaml \
    --arm cond2_diff_q4 \
