...
#SBATCH --mem=200G
#SBATCH --job-name eval_m100_ours
...
#
# EVAL mini100 cond3_ours — 5 checkpoints (step 20,40,60,80,100).
...
CKPT_BASE=$REPO/checkpoints/opsd_curriculum/mini100_8b/mini100_cond3_ours_h200
OUTDIR=$REPO/outputs/eval_opsd_curriculum/mini100_cond3_ours_nonthink
...
run_eval() {
  ...
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL mini100_cond3_ours step=$step  dataset=$ds  val_n=$valn  thinking=OFF  TP=2"
  echo "=================================================================="
  if [ ! -d "$ckpt" ]; then
  ...
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_mini100_cond3_ours_step${step}_nonthink_valn${valn}.json"
