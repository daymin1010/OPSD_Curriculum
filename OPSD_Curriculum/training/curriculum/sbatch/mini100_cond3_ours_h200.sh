...
#SBATCH --gres=gpu:2
#SBATCH --mem=200G
#SBATCH --job-name opsd_mini100_cond3_ours_h200
...
# ============================================================
# MINI50 ours — manifest_once fair 1-pass OPSD curriculum (~1/4.5 of q4).
# Qwen3-8B, H200 x2, B_glob=32 via pd2*ga8*ws2.
# Uses stages_cond3_ours_C2_mini100.json (N=3198, T_total~100).
# save_steps=20 → ckpts at 20/40/60/80/100 (stage starts + last).
# ============================================================
set -euo pipefail
...
DIFF_JSON=$STAGES_TIERED/stages_cond2_diff_mini100.json
OURS_JSON=$STAGES_TIERED/stages_cond3_ours_C2_mini100.json
RUN_CONFIG=mini100_cond3_ours_h200
...
if [ ! -f "$ROW" ]; then
  echo "[mini100] missing row table: $ROW ; run phase0 first" >&2
  exit 2
fi
...
accelerate launch \
    ...
    --config configs/mini100_8b_h200.yaml \
    --arm cond3_ours_mini100 \
    --stages_json "$OURS_JSON" \
    --within_stage_order shuffle \
    --tail_policy partial \
    --curriculum_passes 1 \
