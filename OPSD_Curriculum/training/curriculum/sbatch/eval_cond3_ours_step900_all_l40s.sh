...
#SBATCH --mem=200G
#SBATCH --job-name opsd_eval_ours900
...
#
# EVAL cond3_ours checkpoint-900 (final, stage5 complete) — Qwen3-8B + LoRA(r=64) on L40S x2 (TP=2).
...
for DS in aime24 aime25 hmmt25 math500; do
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL cond3_ours_step900  dataset=$DS  val_n=12  temp=1.0  thinking=OFF  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${DS}_cond3_ours_step900_nonthink_valn12.json"
