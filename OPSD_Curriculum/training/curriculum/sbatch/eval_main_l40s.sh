...
#SBATCH --mem=200G
#SBATCH --job-name opsd_cur_eval_main_l40s
...
#
# EVAL ③-A (main) — Qwen3-8B + LoRA(r=64) on L40S (48GB) x2 (TP=2).
...
CKPT=$REPO/checkpoints/opsd_curriculum/full_8b/full_main_h200   # final adapter (step480, top-level)
OUTDIR=$REPO/outputs/eval_opsd_curriculum/main
...
for DS in aime24 aime25 hmmt25; do
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL main  dataset=$DS  val_n=12  temp=1.0  thinking=ON  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --output_file "$OUTDIR/${DS}_main_thinking_valn12.json"
