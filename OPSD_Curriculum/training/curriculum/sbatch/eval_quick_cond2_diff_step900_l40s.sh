...
#
# QUICK EVAL cond2_diff checkpoint-900 (final) — Qwen3-8B + LoRA(r=64) on L40S x2 (TP=2).
...
CKPT=$REPO/eval_ckpts/cond2_diff_step900
OUTDIR=$REPO/outputs/eval_opsd_curriculum/cond2_diff_step900_nonthink
...
run_eval() {
  local ds="$1" valn="$2"
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL cond2_diff_step900  dataset=$ds  val_n=$valn  temp=1.0  thinking=OFF  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_cond2_diff_step900_nonthink_valn${valn}.json"
