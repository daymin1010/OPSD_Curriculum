...
#
# EVAL mini50 cond2_diff — 5 checkpoints (step 10,20,30,40,50).
...
CKPT_BASE=$REPO/checkpoints/opsd_curriculum/mini50_8b/mini50_cond2_diff_h200
OUTDIR=$REPO/outputs/eval_opsd_curriculum/mini50_cond2_diff_nonthink
...
run_eval() {
  ...
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL mini50_cond2_diff step=$step  dataset=$ds  val_n=$valn  thinking=OFF  TP=2"
  echo "=================================================================="
  if [ ! -d "$ckpt" ]; then
  ...
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_mini50_cond2_diff_step${step}_nonthink_valn${valn}.json"
