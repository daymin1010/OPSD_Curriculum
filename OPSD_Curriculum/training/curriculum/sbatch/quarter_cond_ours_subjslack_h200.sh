...
#SBATCH --gres=gpu:2
#SBATCH --mem=200G
#SBATCH --job-name opsd_quarter_cond_ours_subjslack_h200
...
# ============================================================
# quarter ours — subjslack (level_backbone_residual_subject_slack, alpha=2.0)
# Qwen3-8B, H200 x2, B_glob=32 (pd2*ga8*ws2). manifest: q4, arm=ours.
...
DIFF_JSON=$STAGES/stages_cond2_diff_q4.json
OURS_JSON=$STAGES/stages_cond3_ours_subjslack_q4.json
ARM_JSON=$OURS_JSON
RUN_CONFIG=quarter_cond_ours_subjslack_h200
...
if [ ! -f "$ROW" ]; then echo "[quarter] missing row table: $ROW" >&2; exit 2; fi
...
"$REPO/envs/opsd/bin/python" -m accelerate.commands.launch \
    ...
    --main_process_port 12998 \
    train_opsd_curriculum_manifest_once.py \
    --config configs/quarter_8b_h200_subjslack.yaml \
    --arm cond_ours_quarter_subjslack \
