...
#
# QUICK EVAL cond3_ours checkpoint-900 (final) — Qwen3-8B + LoRA(r=64) on L40S x2 (TP=2).
# non-thinking mode, val_n=3 for aime24/aime25/hmmt25, val_n=1 for math500, temp=1.0.
...
CKPT=$REPO/eval_ckpts/cond3_ours_step900
OUTDIR=$REPO/outputs/eval_opsd_curriculum/cond3_ours_step900_nonthink
...
run_eval() {
  local ds="$1" valn="$2"
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL cond3_ours_step900  dataset=$ds  val_n=$valn  temp=1.0  thinking=OFF  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_cond3_ours_step900_nonthink_valn${valn}.json"
