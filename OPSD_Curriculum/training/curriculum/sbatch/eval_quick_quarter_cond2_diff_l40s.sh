...
CKPT_BASE=$REPO/checkpoints/opsd_curriculum/quarter_8b/quarter_cond2_diff_q4_h200
OUTDIR=$REPO/outputs/eval_opsd_curriculum/quarter_cond2_diff_nonthink
...
run_eval() {
  ...
  if [ ! -d "$ckpt" ]; then
    echo "[$(date +%H:%M:%S)] SKIP step=$step dataset=$ds — checkpoint $ckpt not found"
    return 0
  fi
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL quarter_cond2_diff step=$step  dataset=$ds  val_n=$valn  thinking=OFF  TP=2"
  echo "=================================================================="
  echo "=== Adapter dir: $ckpt ==="; ls -1 "$ckpt" | head
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_quarter_cond2_diff_step${step}_nonthink_valn${valn}.json"
  echo "[$(date +%H:%M:%S)] DONE step=$step dataset=$ds"
}

for STEP in 10 50 100 150 220; do
