...
#SBATCH --mem=200G
#SBATCH --job-name opsd_cur_eval_cond2_nt_l40s
...
# non-thinking mode (--no_thinking), val_n=12 (Avg@12), temp=1.0, datasets: aime24/aime25/hmmt25.
# Auto-settings when --no_thinking: top_p=0.8, max_model_len=32768.
# Based on eval_diffonly_l40s.sh + OPSD_original/eval/run_eval_nonthink.sh pattern.
...
OUTDIR=$REPO/outputs/eval_opsd_curriculum/cond2_diff_nonthink
...
for DS in aime24 aime25 hmmt25; do
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL cond2_diff  dataset=$DS  val_n=12  temp=1.0  thinking=OFF  TP=2"
  echo "=================================================================="
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${DS}_cond2_diff_nonthink_valn12.json"
