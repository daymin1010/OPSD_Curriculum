...
#SBATCH --gres=gpu:2
#SBATCH --mem=200G
#SBATCH --job-name opsd_cur_full_diffonly_h200
...
# ============================================================
# FULL ② diff-only — Qwen3-8B on H200 (141GB) ×2 (ws=2). B_glob=32 via
# pd2*ga8*ws2. gpu_mem_util=0.6. T=480 (arm-common). attach_gold=true
# (monitor-only rollout_acc). Curriculum integrity self-checked by the monitor's
# stage_distinct>1 abort guard. Checkpoints every 30 steps (save_total_limit=3)
# to /scratch; resume_from_checkpoint auto-detected (18h walltime backstop).
# ============================================================
set -euo pipefail
