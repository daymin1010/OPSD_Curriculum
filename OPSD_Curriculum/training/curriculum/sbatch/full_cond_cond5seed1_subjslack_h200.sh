...
#SBATCH --gres=gpu:2
#SBATCH --mem=200G
#SBATCH --job-name opsd_full_cond5seed1_subjslack_h200
...
# ============================================================
# full cond5 (diffmatched, seed1) — subjslack control.
# Qwen3-8B, H200 x2, B_glob=32 (pd2*ga8*ws2). T~900. arm=cond5_seed1.
# cond5 = ours' per-(level,stage) counts, subject RANDOMIZED within level.
#   -> identical per-level difficulty schedule as ours; only subject order destroyed.
#   The load-bearing control: ours > cond5 attributes the gain to subject geometry.
# Universe identical to ours/diff (N=28,771); only stage assignment differs.
# ============================================================
set -euo pipefail
...
OURS_JSON=$STAGES/stages_cond3_ours_subjslack.json
COND5_JSON=$STAGES/stages_cond5_diffmatched_seed1.json
ARM_JSON=$COND5_JSON
RUN_CONFIG=full_cond_cond5seed1_subjslack_h200
...
if [ ! -f "$ROW" ]; then echo "[full] missing row table: $ROW" >&2; exit 2; fi

# fairness gate: cond5 must share IDENTICAL universe with ours (re-partition only)
python verify_schedule_manifest_once.py \
  --diff_json "$OURS_JSON" \
  --ours_json "$COND5_JSON" \
  --row_table "$ROW" \
  --B_glob 32 \
  --seed 42 \
...
"$REPO/envs/opsd/bin/python" -m accelerate.commands.launch \
    ...
    --main_process_port 13002 \
    train_opsd_curriculum_manifest_once.py \
    --config configs/full_8b_h200_subjslack.yaml \
    --arm cond5seed1_full_subjslack \
