...
#SBATCH --mem=200G
#SBATCH --job-name eval_m100_diff
...
#
# EVAL mini100 cond2_diff — 5 checkpoints (step 20,40,60,80,100).
...
run_eval() {
  ...
  echo "=================================================================="
  echo "[$(date +%H:%M:%S)] EVAL mini100_cond2_diff step=$step  dataset=$ds  val_n=$valn  thinking=OFF  TP=2"
  echo "=================================================================="
  if [ ! -d "$ckpt" ]; then
  ...
  python "$EVAL" \
    ...
    --no_thinking \
    --output_file "$OUTDIR/${ds}_mini100_cond2_diff_step${step}_nonthink_valn${valn}.json"
