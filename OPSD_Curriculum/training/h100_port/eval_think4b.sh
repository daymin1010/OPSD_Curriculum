#!/bin/bash
# 4B 커리큘럼 THINKING eval — H100. 원저 run_eval.sh 정합(--no_thinking 없음). AIME24/25+HMMT val12, 최종 ckpt만.
#   사용: REPO=/path ./eval_think4b.sh <ARM> ["100 400 650"]
...
OUTDIR="${OUTDIR:-$WORK/outputs/eval/${RUN_PREFIX:-cliff4b}_${ARM}_think}"
TP="${TP:-2}"
...
for STEP in $STEPS; do
  ...
  for dv in "aime24 12" "aime25 12" "hmmt25 12"; do
    ds=$(echo $dv|cut -d' ' -f1); vn=$(echo $dv|cut -d' ' -f2)
    OUT="$OUTDIR/${ds}_${RUN_PREFIX:-cliff4b}_${ARM}_step${STEP}_think_valn${vn}.json"
    ...
    "$ENV_PY" "$EVAL" --base_model "$BASE_MODEL" --checkpoint_dir "$CKPT" \
      ...
      --tensor_parallel_size "$TP" --gpu_memory_utilization 0.9 --output_file "$OUT"
  done
done
