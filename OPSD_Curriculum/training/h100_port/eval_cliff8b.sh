#!/bin/bash
# 8B 커리큘럼 eval — H100, 직접 실행. 포터블. (eval_cliff4b.sh의 8B 버전)
#   사용: REPO=/path ./eval_cliff8b.sh <ARM> ["100 400 650"]
#   기본: 최종 체크포인트만 자동 감지해 eval. 중간 커브 원하면 인자로 전달.
set -euo pipefail
...
export NODE_CACHE="$WORK/cache/eval8b_${ARM}"
...
export TMPDIR="$WORK/cache/etmp8b_${ARM}"; export TEMP=$TMPDIR TMP=$TMPDIR
