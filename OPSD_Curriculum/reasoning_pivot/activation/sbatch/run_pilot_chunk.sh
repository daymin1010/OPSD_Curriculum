#!/bin/bash
#SBATCH --job-name=Z2_pilot_chunk
...
# Generic pilot chunk runner. Usage:
#   sbatch --job-name=Z2_c0 run_pilot_chunk.sh 0
#   sbatch --job-name=Z2_c1 run_pilot_chunk.sh 1   ... (or --dependency=afterok:<jid>)
# Each chunk = ~402 samples (chunk_id round-robin balanced). 2x L40s, WORLD=2.
...
CHUNK="${1:?usage: run_pilot_chunk.sh <chunk_id 0..3>}"
...
OUTDIR=$ACT/outputs/pilot         # shared shifts dir across all chunks
